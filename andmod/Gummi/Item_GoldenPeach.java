...
public class Item_GoldenPeach extends Item_GoldenFruit {

	public Item_GoldenPeach( int itemID, int maxstack, int heal, float saturation ) {
	...
	protected void tryToApplyFoodEffect( ItemStack items, World world, EntityPlayer eplayer ) {
		...
		if ( !world.isRemote ) {
			...
			case 1:
				...
				eplayer.addPotionEffect( new PotionEffect( Potion.resistance.id, 5 * 60 * 20, 0 ) );
				break;
			case 2:
				eplayer.addPotionEffect( new PotionEffect( Potion.regeneration.id, 30 * 20, 1 ) );
				eplayer.addPotionEffect( new PotionEffect( Potion.resistance.id, 5 * 60 * 20, 2 ) );
